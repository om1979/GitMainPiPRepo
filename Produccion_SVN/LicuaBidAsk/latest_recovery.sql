...
--select * from itblPonderado_BidAsk        -- 966
--select * from itblPonderadoFinal_BidAsk --34
...
--CREATE PROCEDURE dbo.spi_LicuadoFinal;12  
  declare 
    @txtDate AS VARCHAR(10) ='20150324'  
   ,@txtCategory AS CHAR(4)  = 'CET'
   ,@fMatFlag AS BIT = 0
   ,@txtBlenderFlag as varchar(10) = 'Ask'
--AS 
...
*/  
  --  BEGIN  
      ...
      --  SET ANSI_WARNINGS OFF  
      	...
      	/*MANEJO DE TABLAS DE TRABAJO (REINICIO) */
				IF OBJECT_ID('tempdb..#itblMarketPositions') IS NOT NULL DROP TABLE #itblMarketPositions
				IF OBJECT_ID('tempdb..#tblTvPlazoCatalog') is not null drop table #tblTvPlazoCatalog
				IF OBJECT_ID('tempdb..#tblHours') is not null drop table #tblHours
				IF OBJECT_ID('tempdb..#tblMaxYTMTime') is not null drop table #tblMaxYTMTime
				IF OBJECT_ID('tempdb..#itblMarketPositions') is not null drop table #itblMarketPositions
				IF OBJECT_ID('tempdb..#tblNewCorros') is not null drop table #tblNewCorros
				IF OBJECT_ID('tempdb..#tblNewRates') is not null drop table #tblNewRates
				IF OBJECT_ID('tempdb..#tblLastRates') is not null drop table #tblLastRates
				IF OBJECT_ID('tempdb..#tblCorros') is not null drop table #tblCorros
				IF OBJECT_ID('tempdb..#tmp_itblPonderado') is not null drop table #tmp_itblPonderado
				IF OBJECT_ID('tempdb..#tblOperations') is not null drop table #tblOperations
				IF OBJECT_ID('tempdb..#tblMinOperation') is not null drop table #tblMinOperation
				IF OBJECT_ID('tempdb..#tblPosturasChoice') is not null drop table #tblPosturasChoice
				IF OBJECT_ID('tempdb..#itblCorroPonderadoSubastas') is not null drop table #itblCorroPonderadoSubastas
				IF OBJECT_ID('tempdb..#itblCorroPonderado') is not null drop table #itblCorroPonderado
				IF OBJECT_ID('tempdb..#tblFillHoles') is not null drop table #tblFillHoles
				IF OBJECT_ID('tempdb..#tblBestDuration') is not null drop table #tblBestDuration
				IF OBJECT_ID('tempdb..#tblBestRates') is not null drop table #tblBestRates
				IF OBJECT_ID('tempdb..#tblPosturas') is not null drop table #tblPosturas
				IF OBJECT_ID('tempdb..#tblCorroCerradoRate') is not null drop table #tblCorroCerradoRate
				IF OBJECT_ID('tempdb..#tblTimes')IS NOT NULL DROP TABLE #tblTimes	
				IF OBJECT_ID('tempdb..#itblPonderado')IS NOT NULL DROP TABLE #itblPonderado
				IF OBJECT_ID('tempdb..#tblMaxEndTime')IS NOT NULL DROP TABLE #tblMaxEndTime
				IF OBJECT_ID('tempdb..#tblMaxBeginTime')IS NOT NULL DROP TABLE #tblMaxBeginTime
...
----DROP TABLE #tblTimes
    ...
    ,dblRate FLOAT PRIMARY KEY ( intTVPlazoId )  
        ...
        SELECT   @intMinTime = txtValue   FROM  itblBlenderParams  
        WHERE  
 ...
 -- Obtengo las operaciones del periodo  
                ...
                INSERT  #itblMarketPositions  
                        ...
                        SELECT  
                           ...
                           ,CASE WHEN p.txtOperation LIKE 'HECHO%'  
                                 THEN 'HECHO'  ELSE p.txtOperation  END AS txtOperation  
        ...
        ELSE   
           ...
           --en caso de ser Licua_Bid o Licua_Ask se borran Compras o Ventas segun bandera 
                           DELETE 
        ...
        IF @fMatFlag = 0   
            BEGIN  
                ...
                INSERT  #tblLastRates  
                        ...
                        FROM  
                            #tblMaxYTMTime m  
                            INNER JOIN itblNodesYTMLevels_BidAskTest  l ( NOLOCK ) ON m.intSerial = l.intSerialYTM  
                                                              AND m.dteTime = l.dteTime  
                            INNER JOIN #tblTvPlazoCatalog c ON m.txtTv = c.txtTv  
                                                              AND m.intPlazo = c.intPlazo  
                        WHERE  
                            l.dteDate = dbo.fun_NextTradingDate(@txtDate, -1,'MX')  
   ...
   --AND n.dblKey = c.dblKey  
                ...
                INSERT  #tblNewCorros  
                        ...
                        SELECT  
                           ...
                           ,CASE WHEN MIN(v.dblAmount) <= MIN(c.dblAmount)  
                                 THEN MIN(v.dblAmount)  
                                 ELSE MIN(c.dblAmount)  END  
                                 ,SUM(v.dblRate) + SUM(c.dblRate)  / COUNT(v.dblAmount) + COUNT(c.dblAmount)  
                        FROM  
                            #tblOperations v  
                            INNER JOIN #tblOperations c ON v.intTVPlazoId = c.intTVPlazoId  
                        WHERE  
 ...
 -- Inserto en itblPonderado   
  ...
  -- Si no obtenemos el ultimo nivel de las posturas  
   ...
   -- declare 
   -- @txtDate AS VARCHAR(10) ='20150310'  
   --,@txtCategory AS CHAR(4)  = 'CET'
   --,@fMatFlag AS BIT = 0
   --,@txtBlenderFlag as varchar(10) = 'BID'
   
   
   --select * from tblBondsPricesRange  as r 
   --,tblIds AS i 
   -- INNER JOIN tblBonds AS b ON i.txtId1 = b.txtId1 
   --inner join itblPonderado_BidAsk as BD
   --on   DATEDIFF(DAY, '20150310', b.dteMaturity)  = BD.intPlazo
   -- where r.dteDate = '20150310'
  ...
  --select top 10 * from tblBondsPricesRange
                                ...
                                SELECT  
                                   ...
                                   ,i.txtTv  
                                   ,DATEDIFF(DAY, @txtDate, b.dteMaturity) AS intPlazo  
                                  -- ,case when BD.txtType  = 'Bid' then  
                                 --  case when txtOperation = 'HECHO' and 
                                ...
                                FROM  
                                   ...
                                   ,tblIds AS i  
                                    ...
                                    INNER JOIN itblPonderado_BidAsk as BD on   DATEDIFF(DAY, '20150310', b.dteMaturity)  = BD.intPlazo  --oaceves   
                                WHERE  
                                    r.dteDate = @txtDate  
  ...
  -- Ajusto la el nivel de los nodos con subasta  
                ...
                FROM  
                    ...
                    INNER JOIN itblPonderadoFinal_BidAsk p 
                    ON s.txtTv = p.txtTv  
