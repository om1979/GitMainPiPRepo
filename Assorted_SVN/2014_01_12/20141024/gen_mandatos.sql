...
Fecha		: 2014-11-24 12:06:19.723
Descripcion	:Corregir campo VCT para reportar dos enteros, 8 decimales  
-----------------------------------------------------------------------   
*/

  
  ALTER     PROCEDURE [dbo].[usp_productos_PiPGenericos];18  
  --DECLARE 
  ----= '20141031' 
  @txtDate AS  DATETIME 
   ...
   IF EXISTS (  
    ...
    BEGIN  
     ...
     SELECT   
      ...
      CASE   
       WHEN txtVCT IS NULL  OR ISNUMERIC(txtVCT) = 0 
        THEN '0000000000'  
       ELSE  
			 SUBSTRING(REPLACE(STR(ROUND(txtVCT,2),16,6),' ','0'),8,2) +SUBSTRING( STR(ROUND(txtVCT,8),16,8),9,20)  
