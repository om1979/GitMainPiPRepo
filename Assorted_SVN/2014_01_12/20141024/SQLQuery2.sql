...
-- Descripcion Modulo 23: GENERAR PRODUCTO COVAF_MANDATOS_MSCI_EURO_XLS  
...
-- Descripcion Modulo 23: se agrega validacion para poner razon social en mayusculas/ sin acentos/ ni apostrofes 
--------------------------------------------------------------------------   
ALTER   PROCEDURE [dbo].[usp_productos_PiPGenericos];23   
--DECLARE     
@txtDate AS DATETIME  --= '20141028'   
...
BEGIN  
  ...
  INSERT @tmp_tblResultsVector (dteDate,txtId1,txtTv,txtEmisora,txtSerie,dblPMO,dblPMOP,txtCurrency,txtCountry,txtIndex,dblPOR,txtID2,txtSector,txtSubSector,txtRamo,txtSubRamo,txtBCT,txtVCT,txtID7,txtID6,txtID3,txtID4,txtRZ)  
   SELECT  
    ...
    NULL AS [CUSIP],  
    NULL AS [SEDOL],  
    SUBSTRING(REPLACE(REPLACE(REPLACE (REPLACE (REPLACE(REPLACE( ic.txtName,''+CHAR(39)+'' ,''), char(218),'U'),char(211),'O'),char(205),'I' ),char(193),'A'),char(201),'E'),0,80) AS [Razon Social]  
 ...
 IF EXISTS (  
  ...
  BEGIN  
   ...
   SELECT   
    ...
    CASE WHEN txtRZ IS NULL THEN ' ' ELSE RTRIM(REPLACE(UPPER(txtRZ),'Ñ','ALT+0209')) END,  
   ...
   ELSE RTRIM(REPLACE(REPLACE(UPPER(c.txtSuperIssuer),'Ñ','ALT+0209'),'' + CHAR(39)+'','')) END
