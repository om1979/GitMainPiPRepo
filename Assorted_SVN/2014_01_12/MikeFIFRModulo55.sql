sp_createVectorFile ;10   --;55 


sp_helptext sp_createVectorFile


------------------------------------------------------------------------------------------------------------------------------------  
-- Autor:    Mike Ramírez  
-- Fecha Modificacion: 16:00 p.m. 2014-03-11  
-- Descripcion:   Modulo 55: Cambios mayores en campos: Titulos Operados, Fecha de Emisión, Fecha Vencimiento, Días por vencer   
------------------------------------------------------------------------------------------------------------------------------------  
CREATE PROCEDURE dbo.sp_createVectorFile;55  
...
BEGIN  
 ...
 IF @txtVectorInfo IN ('all','trad')  
  ...
  SELECT DISTINCT  
   ...
   CASE WHEN a1.txtMOC <> '-' AND a1.txtMOC <> 'NULL' AND a1.txtMOC <> 'NA' THEN ROUND(CAST(a1.txtMOC AS FLOAT),6)   
     ELSE NULL  
