...
-----------------------------------------------------------------------------------------	
  ...
  Creacion:   2014-02-27 11:12:43.323 
   Descripcion:  Procedimiento que genera producto Goldman Sachs :: GS_VectorAnalítico[yyyymmdd]24H.xls Y MD
  Modifica: Miguel
   Descripcion: se retiran dos columnas y se agregan dos columnas Precio Sucio MO y Precio Limpio MO
-----------------------------------------------------------------------------------------
*/

ALTER   PROCEDURE dbo.usp_productos_GoldmanSachs;3  --'20140319','MD'
 @dtedate DATETIME,
 @txtliqtype VARCHAR(5)
...
-----------------
				...
				DECLARE @dblBRLUS2 AS FLOAT
				DECLARE @dblCHFUS2 AS FLOAT
		...
		ELSE
			BEGIN
				...
				SELECT
					...
					END  AS PRS_MO, 
					CASE
						WHEN txtCUR = '[BRL] Real Brasileno (MXN)' THEN (dblPRL * 100) / (CAST(txtVNA AS FLOAT) * @dblBRL)
						WHEN txtCUR = '[CHF] Franco Suizo (MXN)' THEN (dblPRL * 100) / (CAST(txtVNA AS FLOAT) * @dblCHF)  
...
----- 4
		...
		ELSE
			...
			WHERE
				txtLiquidation IN ('MD','MP')
				AND dteDate = @dtedate
				AND txtTv NOT IN ('*C','*CSP','D1','D2','D4','D5','D6','D7','D8','D1SP','D2SP','D4SP','D5SP','D6SP','D7SP','D8SP','SP')
				AND txtTv LIKE '%SP'
...
-----------------
	...
	--declare @txtliqtype VARCHAR(5)
	--SET @dtedate = '20140319'
	--SET @txtliqtype = '24H'-- se incluye por defecto 'MP' debido al tipo de producto
...
INSERT INTO @tmp_AnaAnalitic
	...
	SELECT 
		...
		replace( replace( replace( replace( replace( RTRIM(LTRIM(r.txtNEM)), 'á', 'A' ), 'é', 'E' ), 'í', 'I' ), 'ó', 'O' ), 'ú', 'U' ) ,
		...
		RTRIM(CASE WHEN r.txtISD = '-' OR r.txtISD = 'NA' THEN 'NA' ELSE CONVERT(CHAR(10),CONVERT(DATETIME, ISNULL(r.txtISD, '1900-01-01')),103) END),
		RTRIM(LTRIM([txtTTM])),
		RTRIM(CASE WHEN r.txtMTD = '-' OR r.txtMTD = 'NA' THEN 'NA' ELSE CONVERT(CHAR(10),CONVERT(DATETIME, ISNULL(r.txtMTD, '1900-01-01')),103) END),
		
		CASE WHEN r.txtNOM = '-' OR r.txtNOM = 'NA' THEN 'NA' ELSE STR(ROUND(RTRIM(LTRIM([txtNOM])),12),21,12)END ,
		
		
		
		replace( replace( replace( replace( replace( RTRIM(LTRIM(r.txtCUR)), 'á', 'A' ), 'é', 'E' ), 'í', 'I' ), 'ó', 'O' ), 'ú', 'U' ) ,
		UPPER(r.txtIRCSUBY),
		...
		UPPER(RTRIM(LTRIM([txtSPQ]))),
		UPPER(RTRIM(LTRIM([txtBUR]))),
		...
		CASE WHEN r.txtIHP = '-' OR r.txtIHP = 'NA' THEN 'NA' ELSE 
				CASE ISNUMERIC(r.txtIHP) WHEN 1 THEN   STR(ROUND(RTRIM(LTRIM([txtIHP])),6),15,6) else 'NA' END END ,--BR_PRC_MI2
		...
		--WHEN r.txtTv IN ('BI','IM','IQ','IP','IS','IT','LD','M','M0','MC','MP','S','SC','SP','S0')
		--	THEN (r.txtTv+'GOBFED'+r.txtSerie) ELSE
		--	(r.txtTv+r.txtEmisora+r.txtSerie)
		--END,
		...
		--CASE
		--WHEN txtTv IN ('*C','*CSP') THEN RTRIM(LTRIM([dblPRS])) END
		--WHEN txtTv IN ('D1','D2','D4','D5','D6','D7','D8') THEN CASE 
		--															WHEN txtCUR = '[BRL] Real Brasileno (MXN)' THEN 
		


		FROM dbo.tmp_tblUnifiedPricesReport  AS r
		
		--LEFT OUTER JOIN @tmp_tblPricesMO2 AS p
		--ON
		--	r.txtId1 = p.txtId1
...
UPDATE r SET r.TXTPRS_MO = STR(ROUND(p.dblPRSMO,6),16,6), r.TXTPRL_MO = STR(ROUND(p.dblPRLMO,6),16,6)
FROM @tmp_AnaAnalitic AS r
INNER JOIN @tmp_tblPricesMO3 AS p
ON
	r.txtId1 = p.txtId1
	AND r.txtCUR = p.txtCUR
...
ON
  ...
  FROM @tmp_AnaAnalitic
 )

	SELECT 
	dtedate,txtTv,txtEmisora
,txtSerie,dblPRL,dblPRS
,dblCPD,dblCuponCPA,dblLDR
,txtNEM,txtSEC,txtMOE
,txtMOC,txtISD,txtTTM
,txtMTD,txtNOM,txtCUR
,txtIRCSUBY,txtCYT,txtOSP
,txtCPF,dblTasaCPA,txtDTC
,txtCRL,txtTCR,txtFCR
,txtLPV,txtLPD,txtTHP
,txtLCA,txtLPU,txtBYT
,txtOYT,txtBSP,txtPSP
,txtDPQ,txtSPQ,txtBUR
,txtLIQ,txtDPC,txtWPC
,txtMHP,txtIHP,txtSUS
,txtVOL,txtVO2,txtDMF
,txtDMT,txtCMT,txtVAR
,txtSTD,txtVNA,txtFIQ
,txtDMH,txtDIH,txtSTP
,txtDMC,dblYTM,txtHRQ
,txtDEF,TXTID2,TXTPRS_MO
,TXTPRL_MO
