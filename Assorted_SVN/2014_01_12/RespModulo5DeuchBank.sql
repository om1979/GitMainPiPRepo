...
CREATE   PROCEDURE [dbo].[sp_productos_DEUTSCHEBANK];5   
@dtedate VARCHAR(10)  
--DECLARE @dtedate VARCHAR(10)= '20140630'  
  --sp_productos_DEUTSCHEBANK;5 '20140618'   
 AS  
 BEGIN   
 SET NOCOUNT ON   
 ...
 INSERT @tblGetIrc  
  SELECT    rep.txtId1,owners2.txtDir,SUBSTRING(rep.txtCUR,2,3) ,c.dblValue  
  FROM   
  MxProcesses.dbo.tblOwnersVsProductsDirectives AS owners2   
  INNER  JOIN dbo.tmp_tblUnifiedPricesReport AS Rep  
  ON owners2.txtDir = Rep.txtId1  
  INNER JOIN dbo.tblIrc  AS C  
  ON CASE WHEN SUBSTRING(Rep.txtCUR,2,3) = 'USD' THEN 'UFXU'  
  ELSE SUBSTRING(Rep.txtCUR,2,3) END  = c.txtIRC  
  WHERE    owners2.txtOwnerId = 'db02'  
  AND owners2.txtProductId = 'vpi'  
  AND C.dteDate = (SELECT MAX(dteDate) FROM dbo.tblIrc  AS AA WHERE AA.txtIRC = C.txtIRC  )   
