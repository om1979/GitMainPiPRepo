 ...
 Fecha Creacion: 12:02 a.m. 2012-02-15  
	...
	Fecha:		2014-07-23 12:08:10.943
	Objetivo:	agregar funcion inull a las ultimas columnas del personalizado para evitar nulos en registros 
*/
ALTER  PROCEDURE dbo.sp_productos_BANAMEX;13  
 --DECLARE  
 @txtDate AS DATETIME ,--= ,--'20140718',  
 --DECLARE  
...
BEGIN  
 ...
 INSERT @tblVector (txtSortTv,txtSortEmisora,txtSortSerie,dtedate,txtTv,txtEmisora,txtSerie,dblPRS,dblPRL,dblCPD,dblCuponCPA,dblLDR,txtNEM,txtSEC,txtMOE,txtMOC,txtISD,txtTTM,txtMTD,txtNOM,txtCUR,txtIRCSUBY,txtCYT,txtOSP,txtCPF,dblTasaCPA,txtDTC,txtCRL,
 txtTCR,txtFCR,txtLPV,txtLPD,txtTHP,txtLCA,txtLPU,txtBYT,txtOYT,txtBSP,txtPSP,txtDPQ,txtSPQ,txtBUR,txtLIQ,txtDPC,txtWPC,txtMHP,txtIHP,
 txtSUS,txtVOL,txtVO2,txtDMF,txtDMT,txtCMT,txtVAR,txtSTD,txtVNA,txtFIQ,txtDMH,txtDIH,txtSTP,txtDMC,dblYTM,txtHRQ,txtDEF)  
  SELECT   
    txtTV AS [txtSortTv],  
    ...
    [txtHRQ] AS [txtHRQ],  
    [txtDEF] AS [txtDEF]  
  FROM MxFixIncome.dbo.tmp_tblUnifiedPricesReport_20140718 (NOLOCK)   
 ...
 txtSUS,txtVOL,txtVO2,txtDMF,txtDMT,txtCMT,txtVAR,txtSTD,txtVNA,txtFIQ,txtDMH,txtDIH,txtSTP,txtDMC,dblYTM,txtHRQ,txtDEF)  
  SELECT   
    ...
    [txtHRQ] AS [txtHRQ],  
    [txtDEF] AS [txtDEF]  
  FROM MxFixIncome.dbo.bkp_tmp_TBLUNIFIEDNOTESREPORT20140718 AS i (NOLOCK)  
 ...
 INSERT @tblResult (txtSortTv,txtSortEmisora,txtSortSerie,txtData)  
  SELECT   
   ...
   RTRIM(LTRIM([txtVAR])) + CHAR(9) +   --bien 
   ISNULL(RTRIM(LTRIM([txtSTD])),'NA')  + CHAR(9) +  
   ISNULL(RTRIM(LTRIM([txtVNA])),'NA')  + CHAR(9) +  
   
ISNULL(RTRIM(LTRIM([txtFIQ])),'NA')  + CHAR(9) +
ISNULL(RTRIM(LTRIM([txtDMH])),'NA')  + CHAR(9) +
ISNULL(RTRIM(LTRIM([txtDIH])),'NA')  + CHAR(9) +
ISNULL(RTRIM(LTRIM([txtSTP])),'NA')  + CHAR(9) +
ISNULL(RTRIM(LTRIM([txtDMC])),'NA')  + CHAR(9) +
ISNULL(RTRIM(LTRIM([dblYTM])),'NA')  + CHAR(9) +
   
ISNULL(RTRIM(LTRIM([txtHRQ])),'NA')  + CHAR(9) +
ISNULL(RTRIM(LTRIM([txtDEF])),'NA')
  FROM @tblVector  
