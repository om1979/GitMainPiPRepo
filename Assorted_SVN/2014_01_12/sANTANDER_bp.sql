...
WHERE --txtValor = '\\via-files\MxVprecios\PRODUCTOS\DEFINITIVO\SANTANDER\ACTUAL\'
 txtvalor LIKE '%Curvas%'


SELECT * FROM  dbo.tblActiveX AS TAX
WHERE txtProceso = 'CURVAS_BP_SANTANDER'

--UPDATE     dbo.tblActiveX 
--SET txtValor = 'STD_Curvas[DATE|YYYYMMDD].xls'
--WHERE txtProceso  ='CURVAS_BP_SANTANDER'
--AND txtPropiedad = 'FileName'

sp_productos_SANTANDER;24 '20141210'


Curvas_BP_satander.xls
...
SET fload = 1
WHERE txtPack = 'CUR'
AND txtProduct NOT IN  ('CURVAS_BP_SANTANDER')



WHERE txtProduct = 'CURVAS_BP_SANTANDER'
...
helptextxmodulo 'sp_productos_SANTANDER',24 '20141210'
...
--   Creacion:   2014-12-03 18:18:54.497     
--   Descripcion:   Modulo 24:crear Modulo para reportar curvas basado en SANTANDER_CURVAS    
-------------------------------------------------------------------------------------------      
ALTER   PROCEDURE dbo.sp_productos_SANTANDER;24      
  @txtDate AS DATETIME      
...
BEGIN      
 ...
 DECLARE @tmpLayoutxlCurve TABLE (      
  ...
  Factor CHAR(5),      
  DataType CHAR(3),       
  DataFormat CHAR(20),      fLoad CHAR(1),      
 PRIMARY KEY (Col)      
 )       
 ...
 INSERT @tmpLayoutxlCurve       
		...
		SELECT 'Sheet1' AS [SheetName], 89 AS [Col],'Ipabono BPAG Mensual'  AS [Header],'CURVES' AS [Source],'BGA'    AS [Type],'BP'    AS [SubType],'-999'     AS [Range],'0.01'     AS [Factor], 'DBL' AS [DataType], '0.000000'AS [DataFormat],'1' AS [fLoad]     UNION
		SELECT 'Sheet1' AS [SheetName], 90 AS [Col],'Ipabono BPAG Trimestral'  AS [Header],'CURVES' AS [Source],'BGT'    AS [Type],'BP'    AS [SubType],'-999'     AS [Range],'0.01'    AS [Factor], 'DBL' AS [DataType], '0.000000'AS [DataFormat],'1' AS [fLoad]     
