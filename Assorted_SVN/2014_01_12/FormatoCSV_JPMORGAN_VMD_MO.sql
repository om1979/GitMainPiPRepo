...
--   Modifica: Omar Aceves     
--   Fecha:   2014-09-24 15:03:55.127     
--   Descripcion:  se agrega tabla para contener resultados y titulos asi como cambio en formato a CSV
----------------------------------------------------------------------------------------    
ALTER  PROCEDURE dbo.sp_productos_JPMORGAN;56 
    @txtDate AS DATETIME   ,
...
AND ap.txtLiquidation IN (@txtLiquidation, 'MP')    
 ...
 FROM @tmp_tblResults AS r    
    ...
    SELECT 'Fecha,Tipo Valor,Emisora,Serie,Precio Limpio MD Moneda Original,Precio Sucio MD Moneda Original,Intereses Acumulados ,Cupón Actual,Sobretasa/Tasa de Rendimiento,ISIN'
  ...
  SELECT     
    ...
    [txtSerie]+',' +     
  --PRL MD    
  LTRIM(CASE WHEN txtTv IN ('SWT','*C','*CSP')     
 ...
 LTRIM( CASE WHEN txtTv IN ('SWT','*C','*CSP')     
  ...
  END))+',' +    
    CONVERT(VARCHAR(50),[dblCPA])+',' +     
    CONVERT(VARCHAR(50),[dblRate])+',' +   
 ...
 IF ((SELECT count(*) FROM @tbl_Report) = 1)    
 ...
 ELSE    
  SELECT txtline FROM @tbl_Report ORDER BY intid 
...
WHERE txtProceso = 'JPMORGAN_VMD_MO'
...
----UPDATE dbo.tblActiveX
----SET txtValor = 'JPMVMD_MO[DATE|YYYYMMDD].csv'
----WHERE txtProceso = 'JPMORGAN_VMD_MO'
...
SET txtPack  = 'VECTOR_MONEDA'
WHERE txtProduct = 'JPMORGAN_VMD_MO'
...
--Fecha,Tipo Valor,Emisora,Serie,Precio Limpio MD Moneda Original,Precio Sucio MD Moneda Original,Intereses Acumulados ,Cupón Actual,Sobretasa/Tasa de Rendimiento,ISIN,
...
--txtTipo,
--txtValor,
--txtDescripcion FROM  dbo.tblActiveX
--WHERE txtProceso = 'VALMEX_24_FF_NAL'
...
--0
--XlQuickQuery
--clsJPMORGAN_VEC_MD_MO.Log
--\\pipmxsql\PRODUCCION\MXVPRECIOS\Log\Actual\
--4
--sp_productos_JPMORGAN;56 '[DATE|YYYYMMDD]','MD'
--Template_JPMORGAN_VMD_MO.xls
--\\pipmxsql\PRODUCCION\MXVPRECIOS\Templates\



--sp_productos_VALMEX;9 '[DATE|YYYYMMDD]'
